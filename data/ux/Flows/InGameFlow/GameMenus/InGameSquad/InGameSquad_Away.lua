...
local InGameSquad_Away = {}

local ACT_ADVANCE = "act_advance"
local ACT_BACK = "act_back"

function InGameSquad_Away:new(init)
  local o = init or {}
  setmetatable(o, self)
  self.__index = self
  o.models = {
...
function InGameSquad_Away:showInvalidSquadPopup(squadValidError)
  local errorMsg = ""
  local buttonYes = VirtualButton:new({
    nav = self.nav,
    label = "LTXT_CMN_OK",
...
function InGameSquad_Away:_handleEvent(eventType, data)
...
function InGameSquad_Away:finalize()
