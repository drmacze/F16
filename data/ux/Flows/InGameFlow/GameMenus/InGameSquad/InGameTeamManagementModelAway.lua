...
local InGameTeamManagementModelAway = {}
...
function InGameTeamManagementModelAway:new(init)
  local o = init or {}
  setmetatable(o, self)
  self.__index = self
  o.services = {
...
function InGameTeamManagementModelAway:getPlayers(teamID)
  local playerLineup
  local players = {}
...
function InGameTeamManagementModelAway:comparePlayers(activePlayer, passivePlayer)
  self.playerComparisonData = self:getPlayerComparisonData(activePlayer, passivePlayer)
  self:_publishPlayerComparisonData()
end

function InGameTeamManagementModelAway:swapPlayersByIndex(activeIndex, passiveIndex)
...
function InGameTeamManagementModelAway:getPlayerComparisonData(activePlayer, passivePlayer)
...
function InGameTeamManagementModelAway:isSubValid(activePlayerIndex, passivePlayerIndex, subsNeeded)
  local isValid = true
  local errorMsg = ""
...
function InGameTeamManagementModelAway:validateSquad()
...
function InGameTeamManagementModelAway:forceSubstitution()
...
function InGameTeamManagementModelAway:saveSquad()
  local playerIDs = {}
  do
    do
...
function InGameTeamManagementModelAway:_publishStarting11()
...
function InGameTeamManagementModelAway:_publishSubsAndRes()
  if self.players == nil then
    return
  end
...
function InGameTeamManagementModelAway:_publishChemistryMatrix()
  if self.isOpponentTeam then
...
function InGameTeamManagementModelAway:_publishTeamStats(bindingName)
  ...
  if self.teamID == 111206 then
    self.im.Publish(BND_TEAM_NAME, ("经典传奇"))
  end
  if self.teamID == 112191 then
    self.im.Publish(BND_TEAM_NAME, ("耐克全明星"))
  end
  if self.teamID == 112139 then
    self.im.Publish(BND_TEAM_NAME, ("利雅得胜利"))
  end
  if self.teamID == 112190 then
    self.im.Publish(BND_TEAM_NAME, ("阿迪达斯全明星"))
  end
end

function InGameTeamManagementModelAway:_publishPlayerComparisonData()
  self.im.Publish(BND_PLAYER_COMPARISON_DATA, self.playerComparisonData)
end

function InGameTeamManagementModelAway:_publishTeamRatingLabel()
  local ratingLabel = ("评分:")
  ratingLabel = ratingLabel..""..tostring(self.teamInfo.overall)
  self.im.Publish(BND_TEAM_RATING_LABEL, ratingLabel)
end

function InGameTeamManagementModelAway:getLineUpBySideAndTeamID(teamSide, teamID)
  local lineup = self.services.SquadManagementService.GetCurrentPlayerLineup(teamSide, teamID, 4)
  return lineup
end

function InGameTeamManagementModelAway:finalize()
...
return InGameTeamManagementModelAway