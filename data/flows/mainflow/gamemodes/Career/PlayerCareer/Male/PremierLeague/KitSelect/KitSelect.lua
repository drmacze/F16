...
function KitSelect:new(init)
  ...
  o.currentOptions = o.services.settingsService and o.services.settingsService.GetCurrentOptions() or {}

  o.homeTeamID = currentMatch.HomeTeamID
  o.awayTeamID = currentMatch.AwayTeamID


  o.im.Subscribe(BND_MATCH_INFO, function()

    local stadiumName = o.currentOptions.stadium or "Default Stadium"
    local weather = o.currentOptions.weather or "Clear"
    local difficulty = o.currentOptions.difficulty or "Legendary"
    local halfLength = o.currentOptions.halfLengthData or "6 Minutes"
    local currentDate = os.date("%d %B %Y")

    local hour = tonumber(os.date("%H"))
    local timeOfDay =
        (hour >= 5 and hour < 12) and "Morning" or
        (hour >= 12 and hour < 18) and "Afternoon" or
        "Night"

    local line1 = string.format("%s | %s | %s", stadiumName, currentDate, difficulty)
    local line2 = string.format("%s | %s | %s", halfLength, timeOfDay, weather)

    o.im.Publish(BND_MATCH_INFO, line1 .. "\n" .. line2)
  end)

  o.im.Subscribe(BND_MATCH_HOUR, function()
    o.im.Publish(BND_MATCH_HOUR, os.date("%I:%M %p"))
  end)
...
self.im.Unsubscribe(BND_MATCH_INFO)
  self.im.Unsubscribe(BND_MATCH_HOUR)
