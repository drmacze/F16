...
local BND_AWAY_CHEMISTRY_MATRIX = "bnd_away_chemistry_matrix"
local BND_AWAY_STARTING_11 = "bnd_away_starting_11"
local BND_AWAY_SUBS_AND_RES = "bnd_away_subs_and_res"
local BND_AWAY_TEAM_NAME = "bnd_away_team_name"
local BND_AWAY_TEAM_CREST = "bnd_away_team_crest"
local BND_AWAY_TEAM_MAN = "bnd_away_team_man"
local BND_AWAY_TEAM_RATING = "bnd_away_team_rating"
local BND_AWAY_TEAM_RATING_LABEL = "bnd_away_team_rating_label"
local BND_AWAY_TEAM_OVERALL = "bnd_away_team_overall"
local BND_AWAY_PLAYER_COMPARISON_DATA = "bnd_away_player_comparison_data"
...
function TeamManagementModel:new(init)
  ...
  o.im.Subscribe(BND_AWAY_CHEMISTRY_MATRIX, function()
    o:_publishChemistryMatrix()
  end)
  o.im.Subscribe(BND_AWAY_STARTING_11, function()
    o:_publishStarting11()
  end)
  o.im.Subscribe(BND_AWAY_SUBS_AND_RES, function()
    o:_publishSubsAndRes()
  end)
  o.im.Subscribe(BND_AWAY_TEAM_NAME, function()
    o:_publishTeamStats(BND_AWAY_TEAM_NAME)
  end)
  o.im.Subscribe(BND_AWAY_TEAM_CREST, function()
    o:_publishTeamStats(BND_AWAY_TEAM_CREST)
  end)
  o.im.Subscribe(BND_AWAY_TEAM_MAN, function()
    o:_publishTeamStats(BND_AWAY_TEAM_MAN)
  end)
  o.im.Subscribe(BND_AWAY_TEAM_RATING, function()
    o:_publishTeamStats(BND_AWAY_TEAM_RATING)
  end)
  o.im.Subscribe(BND_AWAY_TEAM_RATING_LABEL, function()
    o:_publishTeamRatingLabel()
  end)
  o.im.Subscribe(BND_AWAY_TEAM_OVERALL, function()
    o:_publishTeamStats(BND_AWAY_TEAM_OVERALL)
  end)
  o.im.Subscribe(BND_AWAY_PLAYER_COMPARISON_DATA, function()
...
function TeamManagementModel:_publishStarting11()
  ...
  self.im.Publish(BND_AWAY_STARTING_11, {data = starting11})
end
function TeamManagementModel:_publishSubsAndRes()
  ...
  self.im.Publish(BND_AWAY_SUBS_AND_RES, subsAndRes)
end
function TeamManagementModel:_publishChemistryMatrix()
  ...
  self.im.Publish(BND_AWAY_CHEMISTRY_MATRIX, {
...
function TeamManagementModel:_publishTeamStats(bindingName)
  if bindingName == nil then
    self:_publishTeamStats(BND_AWAY_TEAM_NAME)
    self:_publishTeamStats(BND_AWAY_TEAM_CREST)
    self:_publishTeamStats(BND_AWAY_TEAM_MAN)
    self:_publishTeamStats(BND_AWAY_TEAM_RATING)
    self:_publishTeamStats(BND_AWAY_TEAM_OVERALL)
  elseif bindingName == BND_AWAY_TEAM_NAME then
    self.im.Publish(BND_AWAY_TEAM_NAME, self.loc.LocalizeString("TeamName_Abbr15_" .. self.teamID))
  elseif bindingName == BND_AWAY_TEAM_CREST then
    self.im.Publish(BND_AWAY_TEAM_CREST, {
      name = "$Crest",
      id = self.teamID
    })
  elseif bindingName == BND_AWAY_TEAM_MAN then
    self.im.Publish(BND_AWAY_TEAM_MAN, {
      name = "$ManagerCard",
      id = self.teamID
    })
  elseif bindingName == BND_AWAY_TEAM_RATING then
    self.im.Publish(BND_AWAY_TEAM_RATING, self.teamInfo.starRating)
  elseif bindingName == BND_AWAY_TEAM_OVERALL then
    self.im.Publish(BND_AWAY_TEAM_OVERALL, self.teamInfo.overall)
  else
    print("[TeamManagementModel]: _publishTeamStats(): Team stat unknown.")
  end
end
function TeamManagementModel:_publishPlayerComparisonData()
  self.im.Publish(BND_AWAY_PLAYER_COMPARISON_DATA, self.playerComparisonData)
end
function TeamManagementModel:_publishTeamRatingLabel()
  ...
  self.im.Publish(BND_AWAY_TEAM_RATING_LABEL, ratingLabel)
end
function TeamManagementModel:finalize()
  ...
  self.im.Unsubscribe(BND_AWAY_CHEMISTRY_MATRIX)
  self.im.Unsubscribe(BND_AWAY_STARTING_11)
  self.im.Unsubscribe(BND_AWAY_SUBS_AND_RES)
  self.im.Unsubscribe(BND_AWAY_TEAM_NAME)
  self.im.Unsubscribe(BND_AWAY_TEAM_CREST)
  self.im.Unsubscribe(BND_AWAY_TEAM_MAN)
  self.im.Unsubscribe(BND_AWAY_TEAM_RATING)
  self.im.Unsubscribe(BND_AWAY_TEAM_RATING_LABEL)
  self.im.Unsubscribe(BND_AWAY_TEAM_OVERALL)
  self.im.Unsubscribe(BND_AWAY_PLAYER_COMPARISON_DATA)
end
